...
class AttendanceRepository:
    def get_attendance(self, db: Session, attendance_id: int) -> Optional[orm_attendance]:
        return db.query(orm_attendance).filter(orm_attendance.id == attendance_id).first()

    def get_attendances_by_user(self, db: Session, user_id: int) -> List[orm_attendance]:
        return db.query(orm_attendance).filter(orm_attendance.user_id == user_id).all()

    def create_attendance(self, db: Session, attendance: pydantic_attendance.AttendanceCreate, user_id: int) -> orm_attendance:
